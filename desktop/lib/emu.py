...
def generate_alignments(in_file_list, out_basename, database, minimap_type, threads, N, K):
    ...
    minimap2_path = os.path.join(ROOT, "lib", "minimap2", "minimap2")
    command = [
        minimap2_path,
        "-ax", minimap_type,
        "-t", str(threads),
    ...
    for file in os.listdir(dir_path):
        ...
        if file_extension == '.tsv' and 'rel-abundance' in file:
            ...
            if rank in df_sample.columns and metric in df_sample.columns:
                keep_ranks_sample = [value for value in keep_ranks if
                                     value in set(df_sample.columns)]  # check which keep_ranks are in df_sample
                ...
                df_sample_reduced = df_sample_reduced.groupby(keep_ranks_sample,
                                                              dropna=False).sum().reset_index()  # sum metric within df_sample_reduced if same tax lineage
