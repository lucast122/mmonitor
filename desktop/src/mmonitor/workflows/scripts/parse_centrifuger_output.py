...
def parse_fastq_stats(stats_file: Path) -> dict:
    """Parse seqkit stats output (including quality and GC metrics)."""
    ...
    return {
        ...
        'mean_quality_score': float(row.get('AvgQual', 0)),
        'mean_gc_content': float(row.get('GC(%)', 0)),
        'q20_score': float(row.get('Q20(%)', 0)),
        'q30_score': float(row.get('Q30(%)', 0)),
