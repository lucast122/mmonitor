...
from users.models import MAG, KeggMap
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--user-id', type=int, required=True,
                          help='User ID to associate the MAGs with')
        ...
        for mag_dir in os.listdir(results_dir):
            ...
            if os.path.exists(checkm2_path):
                ...
                mag, created = MAG.objects.update_or_create(
                    ...
                    user=user,
                    defaults={
                        'fasta_file': os.path.join(mag_path, f"{mag_dir}.fasta"),
                        'gff_file': os.path.join(mag_path, f"{mag_dir}.gff"),
                        'fai_file': os.path.join(mag_path, f"{mag_dir}.fasta.fai"),
                        ...
                        'sample_name': mag_dir,  # You might want to extract this from somewhere else
                        'taxonomy': checkm2_data.get('taxonomy', 'unknown')
                    }
                )
                ...
                if os.path.exists(annotation_summary):
                    ...
                    mag.gene_count = ann_data.get('gene_count', 0)
                    mag.trna_count = ann_data.get('trna_count', 0)
                    mag.rrna_count = ann_data.get('rrna_count', 0)
                    mag.cds_count = ann_data.get('cds_count', 0)
                    mag.annotation_summary = json.dumps(ann_data)
                    mag.save()
